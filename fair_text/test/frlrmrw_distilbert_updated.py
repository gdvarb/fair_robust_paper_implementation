...
def generate_adversarial_examples(texts, perturb_percent=0.2):
    adv_texts = []
...
def evaluate(model, data_loader, num_classes, perturb_percent=0.2, device='cpu'):
    model.eval()
...
def frlrmrw_train(model, tokenizer, train_loader, valid_loader, optimizer, scheduler, epoch, num_classes,
    ...
    for i in range(num_classes):
        ...
        bndy_vio = class_bndy_error[i] - total_bndy_error
        lmbda_adv[i] = torch.clamp(lmbda_adv[i] + rate2 * adv_vio + rate2 * bndy_vio, min=0, max=lim)
...
def main():
    ...
    # FRLRMRW hyperparameters
    beta = 1.0
    bound0 = 0.07  # Fairness bound for clean error (modified from FRL's 0.05)
    bound1 = 0.07  # Fairness bound for adversarial error (modified from FRL's 0.05)
    rate1 = 0.01   # Update rate for clean fairness
    rate2 = 0.2    # Update rate for adversarial fairness (modified from FRL's 0.05)
    lim = 0.5      # Limit for lambda values
    perturb_percent = 0.2  # Increased from FRL's 0.15
